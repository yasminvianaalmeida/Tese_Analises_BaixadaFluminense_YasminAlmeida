...
# Configura o Tesseract UMA vez, no inicio, e avisa em alto e bom som se ele nao estiver
# disponivel -- em vez de descobrir isso la' na frente, no meio do processamento.
_OCR_OK, _OCR_INFO = _ocr_env.configurar_ocr()
print(("[OCR] disponivel: " + _OCR_INFO) if _OCR_OK
...
# Planilha de parametros (Parametros_LDO_LOA.xlsx), usada mais abaixo para reler a aba
...
# ====================================================================================
R=json.load(open("resultados.json", encoding="utf-8")); OCC=json.load(open("ocorrencias.json", encoding="utf-8"))
...
def _texto_direto(path, npg):
    """Extrai o texto ja' embutido no PDF (sem OCR) das primeiras `npg` paginas."""
    d = fitz.open(path)
    try:
        t = " ".join(d[i].get_text() for i in range(min(npg, d.page_count)))
    finally:
        d.close()
    return norm(t)


def _gerar_cache_ocr(path, cache_path, npg=4):
    """Gera o cache de OCR das primeiras `npg` paginas de um PDF escaneado.

    Os dois arquivos de cache usados aqui (cache_pgv_nilo.json / cache_pd_nilo.json)
    foram gerados manualmente no ambiente onde a analise da tese foi feita e nao vieram
    junto com o pacote de codigo. Esta funcao recria apenas as poucas paginas que a
    datacao realmente usa -- nao o PDF inteiro -- com as mesmas configuracoes de OCR do
    resto do pipeline (zoom 3.0, --oem 1 --psm 3, mesmo idioma de OCR_LANG), e grava o
    resultado em disco para nao repetir o OCR em execucoes futuras na mesma pasta.
    """
    import io as _io
    ...
    d = fitz.open(path)
    try:
        for i in range(min(npg, d.page_count)):
            pix = d[i].get_pixmap(matrix=fitz.Matrix(3.0, 3.0))
            img = _Image.open(_io.BytesIO(pix.tobytes("png")))
            txt = _pytesseract.image_to_string(
                img, lang=os.environ.get("OCR_LANG", "eng"), config="--oem 1 --psm 3")
            paginas[str(i)] = norm(txt)
    finally:
        d.close()
    with open(cache_path, "w", encoding="utf-8") as fh:
        json.dump({"paginas": paginas}, fh, ensure_ascii=False)
    return paginas


def first_text(path, fn, npg=4):
    """Texto das primeiras paginas, usado so' para descobrir o ano do instrumento.

    Estes dois documentos de Nilopolis sao muito escaneados: a extracao direta de texto
    rende pouco ou nada nas primeiras paginas, por isso o codigo original usava um cache
    de OCR. A ordem aqui e': cache pronto -> OCR na hora -> extracao direta como ultimo
    recurso. Nenhum desses caminhos derruba o pipeline: no pior caso a datacao DESTE
    documento fica de baixa confianca e o aviso aparece no log.
    """
    if fn not in cache:
        return _texto_direto(path, npg)

    cache_path = cache[fn]
    if os.path.exists(cache_path):
        c = json.load(open(cache_path, encoding="utf-8"))["paginas"]
        return " ".join(c[str(k)] for k in sorted(int(x) for x in c)[:npg])

    if _OCR_OK:
        try:
            print(f"   [OCR] gerando cache das {npg} primeiras paginas de {fn} ...")
            c = _gerar_cache_ocr(path, cache_path, npg=npg)
            return " ".join(c[str(k)] for k in sorted(int(x) for x in c)[:npg])
        except Exception as e:
            print(f"   AVISO: o OCR falhou em '{fn}' ({e}). "
                  f"Usando a extracao direta de texto como alternativa.")
    else:
        print(f"   AVISO: OCR indisponivel; usando extracao direta de texto em '{fn}'.")

    txt = _texto_direto(path, npg)
    if len(txt.strip()) < 200:
        print(f"   ATENCAO: sem OCR, '{fn}' rendeu apenas {len(txt.strip())} caracteres de "
              f"texto. A datacao deste documento vai ficar de baixa confianca -- confira o "
              f"campo 'ano_evidencia' dele na planilha final.")
    return txt
...
# --- conciliacao: todo instrumento classificado na Etapa 1 precisa ter um registro de
# datacao aqui. Se um PDF foi analisado na Etapa 1 mas nao aparece mais na pasta agora
# (renomeado, movido, ou pasta de origem trocada entre as etapas), o codigo original
# quebraria com KeyError la' na frente. Em vez disso, registramos o instrumento com
# datacao vazia e avisamos, para o pipeline terminar e a lacuna ficar visivel.
_sem_datacao = [fn for fn in inst if fn not in datas]
if _sem_datacao:
    print(f"AVISO: {len(_sem_datacao)} arquivo(s) foram analisados na Etapa 1 mas nao foram "
          f"encontrados agora na pasta de PDFs ({PASTA}):")
    for fn in _sem_datacao:
        print(f"   - {fn}")
        datas[fn] = {"ano": None, "conf": "baixa", "inst_y": None, "capa_max": None,
                     "meta": None,
                     "evidencia": "arquivo nao localizado na pasta de PDFs durante a datacao"}
...
json.dump(out,open("analise_final.json","w",encoding="utf-8"),ensure_ascii=False,indent=2)
